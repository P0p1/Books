...
def login():
    ...
    # reach User route via POST
    if request.method == "POST":

        # check if the user name was submitted
    ...
    # User reach the home page route via GET
    else:
...
def logout():
    ...
    # When the user logout Forget the user ID
    session.clear()

    # This path redirect user to login form
    return redirect("/")
...
def search():
    ...
    # When the books is not founded
    if rows.rowcount == 0:
        return render_template("error.html", message="No description availlaible for this book.")
...
def book(isbn):
    ...
    # Take the book ISBN and redirect to his page (GET)
        ...
        key = os.getenv("betEC3WKSLD1xZdKm3vYA")
...
def api_call(isbn):
    ...
    #""""""""""""""""""""""""""""""""""""""""""""""""""
    # COUNT the rows                                  #
    # SUM returns summarise the cells'values          #
